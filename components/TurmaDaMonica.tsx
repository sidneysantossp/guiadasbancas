...
export default function TurmaDaMonica() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        // Buscar produtos da Turma da Mônica (HQs e Comics)
        ...
        if (r.ok) {
          ...
          // Filtrar produtos da Turma da Mônica (por nome)
          let filtered = allProducts.filter((p: ApiProduct) => {
            const name = p.name?.toLowerCase() || '';
            return (
              p.images && 
              p.images.length > 0 &&
              (name.includes('turma') || name.includes('mônica') || name.includes('monica') || 
               name.includes('cebolinha') || name.includes('cascão') || name.includes('magali'))
            );
          });
          ...
          console.log(`[TurmaDaMonica] Total encontrados: ${filtered.length}`);
        }
      ...
      <div className="container-max">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg sm:text-xl font-semibold">Turma da Mônica</h2>
          </div>
