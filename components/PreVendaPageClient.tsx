...
import { preVendaSeed, type PreVendaProduct } from "@/data/pre-venda";

type PreVendaPageClientProps = {
  initialItems?: PreVendaProduct[];
};
...
function PreVendaPageContent({ initialItems }: PreVendaPageClientProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const initialList = Array.isArray(initialItems) ? initialItems : preVendaSeed;
  const [all, setAll] = useState<PreVendaProduct[]>(initialList);
  ...
  useEffect(() => {
    setAll(initialList);
    setLoading(false);
    try { localStorage.setItem("gb:presale", JSON.stringify(initialList)); } catch {}
  }, [initialList]);
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  onClick={() => {
                    setQ(""); setCat(""); setOrder("relevance"); setDateStart(""); setDateEnd(""); setPage(1);
                    router.replace("/pre-venda" as any);
                  }}
                >
                  Limpar filtros
...
export default function PreVendaPageClient(props: PreVendaPageClientProps) {
  return (
    <Suspense fallback={
