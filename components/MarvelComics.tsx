...
export default function MarvelComics() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        // Buscar produtos da Marvel (HQs e Comics)
        ...
        if (r.ok) {
          ...
          // Filtrar produtos da Marvel (por nome)
          let filtered = allProducts.filter((p: ApiProduct) => {
            const name = p.name?.toLowerCase() || '';
            return (
              p.images && 
              p.images.length > 0 &&
              (name.includes('marvel') || name.includes('homem-aranha') || name.includes('homem aranha') || 
               name.includes('spider') || name.includes('vingadores') || name.includes('avengers') ||
               name.includes('hulk') || name.includes('capitão') || name.includes('thor') || 
               name.includes('iron man') || name.includes('x-men'))
            );
          });
          ...
          console.log(`[MarvelComics] Total encontrados: ${filtered.length}`);
        }
