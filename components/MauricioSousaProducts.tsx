        ...
        <div className="absolute inset-0 p-2">
          <div className="relative h-full w-full rounded-[14px] overflow-hidden">
            ...
            <Link
              ...
              className="absolute inset-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-500"
            />
            <div className="pointer-events-none absolute inset-0 bg-black/0 group-hover:bg-black/5 transition" />
            {discount > 0 && (
              <span className="absolute left-2 top-2 z-10 inline-flex items-center rounded-md bg-yellow-500 text-white px-2 py-[2px] text-[11px] font-semibold shadow">
                -{discount}%
              </span>
            )}
          </div>
        ...
        <div className="mt-auto pt-2 flex flex-col gap-2">
          <div className="flex flex-col gap-0.5">
            {oldPrice && isFinite(oldPrice) ? (
              <>
                ...
                <div className="text-[18px] text-yellow-600 font-extrabold">Por: R$ {price.toFixed(2)}</div>
                {installment && (
              ...
              <>
                <div className="text-[18px] text-yellow-600 font-extrabold">R$ {price.toFixed(2)}</div>
                {installment && (
                  <div className="text-[11px] text-gray-600">10x R$ {Number(installment.toFixed(2)).toFixed(2)} sem juros</div>
                )}
              </>
          ...
          <div className="flex flex-col gap-1">
            <button
              ...
              onClick={handleAddToCart}
              aria-label="Adicionar ao carrinho"
              className="w-full rounded px-2.5 py-1 text-[11px] font-semibold bg-yellow-500 text-white hover:opacity-95"
            >
...
export default function MauricioSousaProducts() {
  ...
  const [apiItems, setApiItems] = useState<MauricioProduct[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const mapped: MauricioProduct[] = merged
          .filter(p => {
    ...
    <section className="w-full pt-8 md:pt-10">
      <div className="container-max">
        <div className="mb-3 flex items-center gap-2">
          <TurmaIcon />
          <h2 className="text-lg sm:text-xl font-semibold text-yellow-600">Maurício de Sousa</h2>
        </div>
