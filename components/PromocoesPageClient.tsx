...
type PromocoesPageClientProps = {
  initialPromos?: Product[];
};

function mapPromos(data: any[]): Product[] {
...
function PromocoesPageContent({ initialPromos }: PromocoesPageClientProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const hasInitial = Array.isArray(initialPromos);
  const [all, setAll] = useState<Product[]>(initialPromos ?? []);
...
export default function PromocoesPageClient(props: PromocoesPageClientProps) {
  return (
    <Suspense fallback={
