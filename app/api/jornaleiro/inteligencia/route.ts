...
type TimelinePeriod = "7d" | "30d" | "90d" | "all";
...
function hasOpeningHours(banca: any) {
  ...
  if (banca?.opening_hours && typeof banca.opening_hours === "object") {
    return Object.values(banca.opening_hours).some((value) => typeof value === "string" && value.trim().length > 0);
  }

  return false;
}

function buildAlert(id: string, tone: "warning" | "critical" | "info" | "success", title: string, description: string, href: string) {
  return { id, tone, title, description, href };
}

function buildRecommendation(id: string, title: string, description: string, href: string, priority: number) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const period = (searchParams.get("period") || "30d") as TimelinePeriod;
    ...
    const topProductsMap = new Map<string, { views: number; clicks: number; cart: number; whatsapp: number }>();
    ...
    for (const event of analyticsEvents) {
      ...
      const current = topProductsMap.get(event.product_id) || { views: 0, clicks: 0, cart: 0, whatsapp: 0 };
    ...
    const checklistCompleted = [hasBranding, openingHoursConfigured, hasContactChannel, hasOwnProducts].filter(Boolean).length;
    const isPublished = Boolean(banca.active && banca.approved);
    const activeCampaigns = campaigns.filter((item) => ["active", "approved"].includes(String(item.status || "").toLowerCase())).length;
    const pendingCampaigns = campaigns.filter((item) => String(item.status || "").toLowerCase() === "pending").length;

    const catalogCoverageRate =
      activeOwnProducts.length > 0
        ? Math.max(0, Math.round(((activeOwnProducts.length - productsWithoutImage) / activeOwnProducts.length) * 100))
        : 0;
    ...
    const alerts = [
      ...
      entitlements.subscription?.status === "overdue"
        ? buildAlert(
            ...
            entitlements.overdueFeaturesLocked ? "Recursos pagos estao pausados" : "Existe cobranca em aberto do plano",
    ...
    const recommendations = [
      ...
      entitlements.canAccessDistributorCatalog
        ...
        : null,
    ].filter((item): item is { id: string; title: string; description: string; href: string; priority: number } => Boolean(item))
