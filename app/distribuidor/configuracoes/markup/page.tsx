...
export default function MarkupConfigPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {/* Markup Global - Seção Simplificada */}
      <div className="bg-white rounded-xl shadow-sm border p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
          <IconPercentage className="text-green-600" size={24} />
          Markup Global
        </h2>
        <p className="text-sm text-gray-600 mb-4">
          Defina um markup padrão que será aplicado a <strong>todos os produtos</strong> que não possuem markup específico por categoria ou produto.
        </p>

        {/* Markup Global atual */}
        {(globalPercentual > 0 || globalFixo > 0) && (
          <div className="mb-4 p-3 bg-green-50 rounded-lg flex items-center justify-between">
            <div>
              <span className="font-medium text-green-800">Markup Global Ativo:</span>
              <span className="ml-2 text-green-700">
                {globalPercentual > 0 && `${globalPercentual}%`}
                {globalPercentual > 0 && globalFixo > 0 && ' + '}
                {globalFixo > 0 && formatPrice(globalFixo)}
              </span>
              <span className="ml-3 text-sm text-green-600">
                (Exemplo: R$ 100,00 → {formatPrice(calcularPrecoFinal(100, globalPercentual, globalFixo))})
              </span>
            </div>
          </div>
        )}

        {/* Formulário de Markup Global Simples */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div className="md:col-span-2">
            <div className="p-3 bg-gray-50 rounded-lg text-sm text-gray-600">
              <strong>Fórmula:</strong> Preço Final = Preço Base × (1 + %) + Fixo
            </div>
          </div>
          <div>
            <input
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={saveGlobalMarkup}
              disabled={saving}
              className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 h-fit flex items-center gap-1"
            >
              <IconDeviceFloppy size={18} />
              Salvar
            </button>
        ...
        <div className="mt-4 pt-4 border-t flex flex-wrap gap-2">
          <button
            onClick={applyBulkMarkup}
            disabled={saving || applyingBulk || totalProdutos === 0}
            className="px-4 py-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 disabled:opacity-50 flex items-center gap-2 text-sm font-medium"
          >
            <IconWand size={16} />
            {applyingBulk ? 'Aplicando...' : `Aplicar em ${totalProdutos} produtos`}
          </button>
          {markupProdutos.length > 0 && (
            <button
              onClick={clearAllProductMarkups}
              disabled={saving || applyingBulk}
              className="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 disabled:opacity-50 flex items-center gap-2 text-sm font-medium"
            >
              <IconRefresh size={16} />
              Resetar {markupProdutos.length} markups individuais
            </button>
          )}
        </div>
      </div>
