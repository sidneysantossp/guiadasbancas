        ...
        <div className="space-y-3">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-2">
            ...
            <div>
              ...
              <select name="category" className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
                <option value="">Selecione uma categoria</option>
                {categories.map((c) => (
                  <option key={c.id} value={c.id}>{c.name}</option>
                ))}
              </select>
              <p className="text-xs text-gray-500 mt-1">Deixe em branco se não se aplicar</p>
            </div>
