...
async function test() {
  ...
  while (true) {
    ...
    const activeInPage = data.filter(p => p.ativo !== false && (p.saldo_estoque || 0) > 0);
    totalActive += activeInPage.length;
  ...
  console.log(`Total de produtos ativos na Mercos (ativo e com estoque > 0): ${totalActive}`);
}
test();
