...
async function count() {
  ...
  let totalActiveFlag = 0;
  let totalActiveAndStock = 0;
  let page = 1;
  ...
  while (true) {
    ...
    totalActiveFlag += data.filter(p => p.ativo !== false && p.excluido !== true).length;
    totalActiveAndStock += data.filter(p => p.ativo !== false && p.excluido !== true && (p.saldo_estoque || 0) > 0).length;
  ...
  console.log(`Produtos apenas com flag ativo=true (independente de estoque): ${totalActiveFlag}`);
  console.log(`Produtos ativos E com estoque > 0: ${totalActiveAndStock}`);
}
count();
